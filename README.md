...
# run app
java -jar target/javaeichenberger-1.0-SNAPSHOT.jar
...
docker buildx build   --platform linux/amd64   -t parsontodd/javaeichenberger:latest   --push .
```
...
# watch rollout
kubectl rollout status deploy/javaeichenberger -n care

# get external endpoint
kubectl get svc javaeichenberger-svc -n care
```
...
```bash
kubectl port-forward svc/javaeichenberger-svc 8080:80 -n care
...
kubectl rollout restart deploy/javaeichenberger -n care
kubectl rollout status deploy/javaeichenberger -n care
```
...
info.app.name=javaeichenberger
info.app.description=Eichenberger
```
...
├─ src/
│  ├─ main/
│  │  ├─ java/com/harness/javaeichenberger/
│  │  │  ├─ App.java
...
│  │        └─ index.html
│  └─ test/
│     └─ java/com/harness/javaeichenberger/AppTest.java
├─ kubernetes/
│  ├─ deployment.yml
│  └─ service.yml
├─ Dockerfile
...
© justin.eichenberger@carelumen.com. For demo/PoV purposes.
